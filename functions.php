...
function bxc_transactions_check($transaction_id) {
    ...
    if ((($transaction_creation_time + $refresh_interval) <= $time) && !$is_demo) {
        return 'expired';
    }
    if ($boxcoin_transaction) {
        ...
        if ($cryptocurrency_code === 'btc_ln') {
            ...
            $address_generation = $to != bxc_settings_get_address($cryptocurrency_code) && !$is_demo;
            ...
            for ($y = 0; $y < count($transactions_blockchain); $y++) {
                ...
                if ((empty($transaction_blockchain['time']) || $transaction_blockchain['time'] > strtotime($transaction['creation_time'] . ' UTC')) && ($address_generation || $transaction['amount'] == $transaction_blockchain['value'] || strpos($transaction_blockchain['value'], $transaction['amount']) === 0)) {
                    ...
                    if ($response && !empty($response['confirmed'])) {
                        bxc_transactions_update($transaction['id'], ['status' => empty($response['underpayment']) ? 'C' : 'X']);
                    }
                }
            }
        }
...
function bxc_crypto_balances($cryptocurrency_code = false) {
    ...
    for ($i = 0; $i < count($cryptocurrencies); $i++) {
        ...
        if (($cryptocurrency_code === 'btc' && bxc_settings_get('btc-node-address-generation')) || bxc_is_eth_address_generation($cryptocurrency_code)) {
            ...
            if ($addresses_db) {
                $addresses_db = json_decode(bxc_encryption($addresses_db, false), true);
                for ($i = 0; $i < count($addresses_db); $i++) {
                    if (isset($addresses_db[$i][0]['address'])) {
                        $addresses_db[$i] = $addresses_db[$i][0]['address'];
                    }
                }
                $addresses = array_merge($addresses_db, $addresses);
            }
        }
        if ($addresses) {
            $balance = 0;
...
function bxc_exchange_rates($currency_code, $cryptocurrency_code) {
    ...
    if (empty($BXC_EXCHANGE_RATE)) {
        $BXC_EXCHANGE_RATE = json_decode(bxc_settings_db('exchange_rate_' . $currency_code), true);
        if (bxc_isset($BXC_EXCHANGE_RATE, 'last_update') < (time() - 86400)) {
            $BXC_EXCHANGE_RATE = [];
        }
    }
    for ($i = 0; $i < 4; $i++) {
        if (empty($BXC_EXCHANGE_RATE) || empty($BXC_EXCHANGE_RATE[$cryptocurrency_code]) || $BXC_EXCHANGE_RATE['currency_code'] != $currency_code) {
            switch ($i) {
                ...
                case 2:
                    ...
                    $value = bxc_isset(bxc_isset($response, $cryptocurrency_code_2), strtolower($currency_code));
                    if ($value) {
                        $BXC_EXCHANGE_RATE[$cryptocurrency_code] = 1 / $value;
                    }
                    break;
                case 3:
            ...
            $BXC_EXCHANGE_RATE['last_update'] = time();
            bxc_settings_db('exchange_rate_' . $currency_code, $BXC_EXCHANGE_RATE);
        } else {
            break;
        }
    }
...
function bxc_crypto_get_decimals($cryptocurrency_code = false) {
    $custom_tokens = bxc_get_custom_tokens();
    $cryptocurrency_code = strtolower($cryptocurrency_code);
    $decimals = ['btc' => 8, 'btc_ln' => 8, 'eth' => 8, 'xrp' => 6, 'usdt' => 6, 'usdt_tron' => 6, 'usdt_bsc' => 6, 'usdc' => 6, 'link' => 5, 'doge' => 8, 'algo' => 6, 'shib' => 1, 'bat' => 3, 'bnb' => 7, 'busd' => 18, 'ltc' => 8, 'bch' => 8, 'sol' => 8, 'xmr' => 8];
    foreach ($custom_tokens as $key => $value) {
...
function bxc_crypto_get_image($cryptocurrency_code) {
    return bxc_crypto_is_custom_token($cryptocurrency_code) ? bxc_get_custom_tokens()[strtolower($cryptocurrency_code)]['img'] : BXC_URL . 'media/icon-' . strtolower($cryptocurrency_code) . '.svg';
}
...
function bxc_login($username, $password, $code = false) {
    ...
    if ($is_success_login) {
        ...
        if (bxc_settings_get('notifications-login')) {
            $language = bxc_settings_get('language-admin');
            bxc_email_notification(bxc_m('New login', $language), str_replace(['{T}', '{T2}', '{T3}'], [BXC_URL . 'admin.php', date('Y-m-d H:i:s'), $ip], bxc_m('New Boxcoin login at the URL {T}. Date and time of access: {T2}. IP: {T3}.', $language)));
        }
...
function bxc_settings_js_admin() {
    ...
    $code = 'var BXC_LANG = "' . $language . '"; var BXC_TRANSLATIONS = ' . ($language ? file_get_contents(__DIR__ . '/resources/languages/admin/' . $language . '.json') : '{}') . '; var BXC_CURRENCY = "' . bxc_settings_get('currency', 'USD') . '"; var BXC_URL = "' . BXC_URL . '"; var BXC_ADMIN = true; var BXC_ADDRESS = ' . json_encode($addresses) . ';';
    $refunds = [];
    $settings = [
        'apps' => [],
...
function bxc_settings_get_address($cryptocurrency_code, $single = true) {
    ...
    $addresses = $address ? explode(',', str_replace(' ', '', preg_replace('/\s+/', '', $address))) : false;
...
function bxc_blockchain($cryptocurrency_code, $action, $extra = false, $address = false) {
    $services = [
        ...
        'bnb' => [['https://api.etherscan.io/v2/api?chainid=56&', 'module=account&action=balance&address={R}', 'module=account&action=txlist&address={R}&startblock=0&endblock=99999999&offset=99&sort=asc', 'module=account&action=txlist&address={R}&startblock=0&endblock=99999999&offset=99&sort=asc', false, 'bscscan']],
    ...
    $services_testnet = [
        ...
        'eth' => [['https://api.etherscan.io/v2/api?chainid=11155111&', 'module=account&action=balance&address={R}', 'module=account&action=txlist&address={R}&startblock=0&endblock=99999999&offset=99&sort=asc', 'module=account&action=txlist&address={R}&startblock=0&endblock=99999999&offset=99&sort=asc', false, 'etherscan']],
    ];
    ...
    if ($is_token) {
        switch ($is_token) {
            case 'eth':
                require_once(__DIR__ . '/web3.php');
                $services = [['https://api.etherscan.io/v2/api?chainid=1&', 'module=account&action=tokenbalance&contractaddress={A}&address={R}&tag=latest', 'module=account&action=tokentx&address={R}&startblock=0&endblock=99999999&offset=99&sort=asc', 'module=account&action=tokentx&address={R}&startblock=0&endblock=99999999&offset=99&sort=asc', false, 'etherscan', 'module=account&action=tokentx&address={R}&startblock=0&endblock=99999999&offset=99&sort=asc'], ['https://api.ethplorer.io/', 'getAddressInfo/{R}', 'getAddressHistory/{R}?limit=99&showZeroValues=false', 'getTxInfo/{R}', false, 'ethplorer', 'getAddressHistory/{R}?limit=99&showZeroValues=false'], ['https://blockscout.com/eth/mainnet/api?', 'module=account&action=tokenbalance&contractaddress={A}&address={R}', 'module=account&action=tokentx&address={R}&offset=99', 'module=account&action=tokentx&address={R}&offset=99', false, 'blockscout', 'module=account&action=tokenlist&address={R}']];
            ...
            case 'trx':
                ...
                break;
            case 'bsc':
                $services = [['https://api.etherscan.io/v2/api?chainid=56&', 'module=account&action=tokenbalance&contractaddress={A}&address={R}&tag=latest', 'module=account&action=tokentx&contractaddress={A}&address={R}&startblock=0&endblock=99999999&offset=99&sort=asc', 'module=account&action=tokentx&contractaddress={A}&address={R}&startblock=0&endblock=99999999&offset=99&sort=asc', false, 'bscscan', 'module=account&action=tokentx&address={R}&startblock=0&endblock=99999999&offset=99&sort=asc']];
                    ...
                    case 'balance':
                        if ($cryptocurrency_code == 'usdt_tron') {
                            ...
                            if ($is_token) {
                                ...
                                if ($trc_20) {
                                    foreach ($trc_20 as $trc_20_) {
                                        if (isset($trc_20_[$contract_address])) {
                                            return bxc_decimal_number($trc_20_[$contract_address] / (10 ** bxc_crypto_get_decimals($cryptocurrency_code)));
                                        }
                                    }
                                }
                            ...
                            $json = bxc_curl($base_url . 'address/balance/' . $address, '', $header);
                            $data = json_decode($json, true);
...
function bxc_paypal_get_checkout_url($transaction_id, $checkout_url, $amount, $currency_code, $title = '') {
    ...
    $data = [
        ...
        'custom' => $transaction_id . (0 ? '|' . bxc_cloud_get_data() : '')
    ];
...
function bxc_cloud_spend_credit($transaction_amount, $currency) {
    $fee_usd = floatval($transaction_amount) * bxc_cloud_get_fee($transaction_amount);
...
function bxc_cloud_get_fee($amount) {
    if (defined('CLOUD_FEE')) {
        if (is_numeric(CLOUD_FEE)) {
            return CLOUD_FEE;
        }
        foreach (CLOUD_FEE as $fee => $range) {
